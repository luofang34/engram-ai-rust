...
///
/// 1. Generate candidates from FTS + vector search
/// 2. RRF-fuse the ranked lists
/// 3. Score with cognitive model blend
/// 4. MMR-select for diversity
/// 5. Filter noise
pub fn retrieve(
    storage: &MemoryStore,
    query: &str,
    ...
    let pool_size = limit * config.candidate_multiplier;

    // 1. Generate candidate lists
    let fts_results = storage.search_fts(query, pool_size)?;
    let fts_ids: Vec<String> = fts_results.iter().map(|r| r.id.clone()).collect();

    let vec_results = if let Some(emb) = query_embedding {
        storage.search_vector(emb, pool_size, 0.05)
    } else {
        Vec::new()
    };
    let vec_ids: Vec<String> = vec_results.iter().map(|(id, _)| id.clone()).collect();

    // Build semantic score map
    let semantic_scores: HashMap<String, f64> = vec_results
        .into_iter()
    ...
    if !vec_ids.is_empty() {
        ranked_lists.push(vec_ids.clone());
    }
    // Also add a cognitive-ranked list if we have scores
    ...
    // Collect all unique candidate records
    ...
    for id in &vec_ids {
        if !candidate_map.contains_key(id) {
            if let Some(r) = storage.get(id)? {
                candidate_map.insert(id.clone(), r);
            }
        }
    }
    for id in cognitive_scores.keys() {
        if !candidate_map.contains_key(id) {
            if let Some(r) = storage.get(id)? {
                candidate_map.insert(id.clone(), r);
            }
    ...
    for (id, rrf_score) in &rrf_ranked {
        ...
        let sem_score = semantic_scores.get(id).copied().unwrap_or(0.0);

        // Normalize cognitive score via sigmoid
        let cog_norm = 1.0 / (1.0 + (-cog_score).exp());

        // Blend: RRF provides the base, modulated by cognitive and semantic signals
        ...
        candidates.push(ScoredCandidate {
            ...
            components: ScoreComponents {
                ...
                vector_rank: vector_ranks.get(id.as_str()).copied().unwrap_or(0),
            },
        });
    }
